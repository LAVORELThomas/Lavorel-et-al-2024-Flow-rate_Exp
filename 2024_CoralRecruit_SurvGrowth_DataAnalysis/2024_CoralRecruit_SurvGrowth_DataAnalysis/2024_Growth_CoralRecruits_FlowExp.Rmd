...
overall_growth_12weeks <- GrowthVelocity_data %>%
  group_by(Flume_Nb) %>%
  summarise(
    n_recruits = n()
  )
...
# Giving the predicted median for 5 and 20 cm s-1 for 7 weeks
pred_points_7weeks <- emmeans(fit1, ~Velocity, at = MinMax_7weeks, type = "link") %>% 
  gather_emmeans_draws() %>% 
...
# Giving the predicted median for 5 and 20 cm s-1 for 12 weeks
pred_points_12weeks <- emmeans(fit11, ~Velocity, at = MinMax_12weeks, type = "link") %>% 
  gather_emmeans_draws() %>% 
  mutate(pred = exp(.value)) %>% 
...
velocity_range <- range(GrowthVelocity_data$Velocity)

# Get posterior draws on response scale
draws_df <- emmeans(fit11, ~Velocity, at = list(Velocity = velocity_range), type = "response") %>%
  gather_emmeans_draws() %>%
...
ggplot(draws_df, aes(x = pred_growth, fill = factor(Velocity), color = factor(Velocity))) +
  ...
  geom_vline(data = summary_df, aes(xintercept = median, color = factor(Velocity)), size = 1.2) +
  geom_text(data = summary_df,
            aes(x = median -0.035, y = 40,  # adjust y to position the text appropriately
...
# Per flumes
flumes <- sort(unique(GrowthVelocity_data$Flume_Nb))
...
draws_df <- add_epred_draws(fit11, newdata = newdata, re_formula = NULL) %>%
  mutate(pred_growth = .epred)

summary_df <- draws_df %>%
  group_by(Flume_Nb, Velocity) %>%
  summarise(
    mean_growth = mean(pred_growth),
    lower = HDInterval::hdi(pred_growth)[1],
    upper = HDInterval::hdi(pred_growth)[2],
    .groups = "drop"
  )
...
```{r}
ratio_draws <- emmeans(fit11, ~ Velocity, at = MinMax_7weeks, type = "link") %>%
  gather_emmeans_draws() %>%
  group_by(.draw) %>%
