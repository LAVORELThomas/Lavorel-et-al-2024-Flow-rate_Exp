...
summary_survival_table %>%
  summarise(
    mean_surv_mean = mean(mean_surv, na.rm = TRUE),
    mean_surv_sd   = sd(mean_surv, na.rm = TRUE)
  )
overall_survival_stats %>%
  summarise(
    mean_surv_mean = mean(mean_surv_7weeks, na.rm = TRUE),
    mean_surv_sd   = sd(mean_surv_7weeks, na.rm = TRUE)
  )
```
...
ggplot(SurvivalvsFlow_Data, aes(x = Velocity, y = All_neighbours)) +
  geom_point(alpha = 0.3) +
...
```
```{r}
# Treat Flume_Nb as factor for discrete colors
SurvivalvsFlow_Data <- SurvivalvsFlow_Data %>%
  mutate(Flume_Nb = as.factor(Flume_Nb))

# Custom colors for each flume
flume_colors <- c("#35978f", "#CAB77E", "#B284BF")

ggplot(SurvivalvsFlow_Data, aes(x = Velocity, y = All_neighbours, color = Flume_Nb)) +
  geom_point(alpha = 0.2, size = 2) +  # Lower alpha for less opaque dots
  geom_smooth(method = "lm", se = TRUE, size = 1) +
  facet_wrap(~Flume_Nb) +
  scale_color_manual(values = flume_colors, name = "Flume Number") +
  theme_minimal(base_size = 14) +
  labs(
    title = "Velocity vs All Neighbours by Flume",
    x = "Velocity (cm/s)",
...
# Unique flumes
flumes <- unique(SurvivalvsFlow_Data$Flume_Nb)

# Function to extract lm stats
get_lm_stats <- function(data) {
  fit <- lm(all_neighbours ~ Velocity, data = data)
  summ <- summary(fit)
  data.frame(
    Flume_Nb = unique(data$Flume_Nb)[1],
    R2 = summ$r.squared,
    slope = coef(fit)[["Velocity"]],
    p_value = summ$coefficients["Velocity", "Pr(>|t|)"]
   )
}

# Stats per flume
per_flume_stats <- do.call(rbind, lapply(flumes, function(f) {
  get_lm_stats(subset(SurvivalvsFlow_Data, Flume_Nb == f))
}))

# Global stats
global_stats <- get_lm_stats(SurvivalvsFlow_Data)
global_stats$Flume_Nb <- "Global"

# Combine
lm_table <- rbind(per_flume_stats, global_stats)
lm_table
```
...
# group by integer of velocity

Grouped_SurvivalvsFlow_Data_12weeks <- SurvivalvsFlow_Data_12weeks %>%
  mutate(Velocity = floor(Velocity)) %>%   # take integer part
  group_by(Velocity, Flume_Nb) %>%
  summarise(
    surv = mean(ind, na.rm = TRUE), 
    sd_surv = sd(ind, na.rm = TRUE),
    n = n(),
  )
...
ggplot(fitted_probs, aes(x = Velocity, y = Estimate)) +
  geom_point(
    data = Grouped_SurvivalvsFlow_Data_12weeks,
    aes(
      x = Velocity,
      y = surv,
    ...
    position = position_jitter(width = 0.2, height = 0),  # jitter only in x
    alpha = 0.6
  ) +
